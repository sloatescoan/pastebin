...
struct PasteController<Context: RequestContext> {
    ...
    @Sendable func getRoot(request: Request, context: Context) async throws -> TemplResponse {
        ...
        if alreadyHasCookie {
            logger.info("\(#fileID):\(#function) Skipping 'remember' UI; request already has a correct cookie.")
        }
    ...
    @Sendable func postPaste(request: Request, context: Context) async throws -> Response {
        ...
        do {
            ...
            logger.info("\(#fileID):\(#function) Received valid input.")
        } catch {
            logger.warning("\(#fileID):\(#function) Received invalid input.")
        ...
        guard
            ...
            logger.warning("\(#fileID):\(#function) Secret is incorrect.")
        ...
        do {
            ...
            logger.info("\(#fileID):\(#function) Wrote to key: \(key)")
        } catch {
            // failed to write
            logger.warning("\(#fileID):\(#function) Could not write to storage. Error: \(error)")
        ...
        if setCookie {
            ...
            logger.info("\(#fileID):\(#function) Setting the secret cookie.")
    ...
    @Sendable func getPaste(request: Request, context: Context) async throws -> TemplResponse {
        ...
        guard let key = context.parameters.get("key") else {
            logger.warning("\(#fileID):\(#function) Could not get key from request.")
            throw HTTPError(.badRequest)
        }
        do {
            logger.info("\(#fileID):\(#function) Fetching key: \(key)")
            let body = try await storage.get(key: key)
            logger.info("\(#fileID):\(#function) Fetched key: \(key)")
            return .init(
                status: .ok,
            ...
            // did not fetch key
            logger.warning("\(#fileID):\(#function) Could not fetch key: \(error)")
